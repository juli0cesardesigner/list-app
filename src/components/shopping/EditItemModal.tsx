...
import { ShoppingItem, parseItemName, MatchedHistoryResult } from "@/types/shopping";
...
interface EditItemModalProps {
  ...
  onClose: () => void;
  onSave: (name: string, quantity: number, alternatives: string[]) => void;
...
export default function EditItemModal({
  ...
  useEffect(() => {
    if (item) {
      ...
      setQuantity(isNaN(qty) ? 1 : qty);
      setAlternatives(
        parsed.alternatives.length > 0 ? parsed.alternatives : [""]
      );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) return;
    onSave(name, quantity, alternatives);
  };
